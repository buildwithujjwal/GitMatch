...
const GeneratedBreakdown = require("../models/BreakdownCache");
...
const getBreakdown = async (req, res) => {
  ...
  // check cache first
  if (issue_url) {
    const cached = await GeneratedBreakdown.findOne({ issue_url });
    if (cached) {
...
module.exports = { saveIssue, removeIssue, showSaved, getBreakdown };
